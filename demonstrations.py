...
def load_demonstrations(data_folder):
    ...
    # actions = []
    # observations = []
    # for doc in listdir(data_folder):
    #     if "action" in doc:
    #         actions.append(np.load(data_folder+'\\' + doc))
    #     elif "observation" in doc:
    #         observations.append(np.load(data_folder+'\\' + doc))\
            # observations.append(np.transpose(np.load(data_folder+'\\' + doc), (2,0,1)))
    from pathlib import Path
    data_path = Path(data_folder).resolve()
    actions = [np.load(i) for i in sorted(data_path.iterdir()) if i.name.startswith('a')]
    observations = [np.transpose(np.load(i), (2,0,1)) for i in sorted(data_path.iterdir()) if i.name.startswith('o')]
...
def save_demonstrations(data_folder, actions, observations):
    ...
    actions:        python list of N numpy.ndarrays of size 3
    """
    np.save(data_folder+'\\'+'actions.npy', observations)
    np.save(data_folder+'\\'+'actions.npy', observations)
