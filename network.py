...
class ClassificationNetwork(torch.nn.Module):
    def __init__(self):
        ...
        self.num_classes = self.action_classes.shape[0]
    ...
    def forward(self, observation, device=None):
        """
        1.1 e)
        ...
        observation = observation.to(device=device, dtype=torch.float)   # make sure input type and NN weight type are the same (torch."device".float)
        # batch_size = observation.shape[0]
        # x = self.vit(observation)
    ...
    def actions_to_classes(self, actions):
        ...
        if type(actions) != list:
            for i in range(self.num_classes):
                if (self.action_classes[i] == actions).all():
                    return torch.tensor([int(i)])

        classes = []
        for a in actions:
            for i in range(self.num_classes):
                if (self.action_classes[i] == a).all():
                    classes.append(torch.tensor([int(i)]))
